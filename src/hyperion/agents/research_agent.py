...
def generate_email(prospect: Dict, hook: str) -> Optional[str]:
    ...
    Args:
        prospect: A dictionary containing prospect information.
        hook: The personalized hook generated by the research phase.

    Returns:
        A formatted string containing the email's subject and body.
    """

    print("\n --- Node: Generating Final Email ---")
    prospect_name = prospect.get('name', '').split(' ')[0]
    ...
    try:
        ...
        prompt = (
            "You are an expert-level Sales Development Representative. Your task is to write a short, professional, and highly personalized cold email.\n\n"
            "**Persona & Tone:**\n"
            "- Be respectful, concise, and professional.\n"
            "- Avoid overly salesy or marketing language.\n"
            "- Write with a confident, human touch.\n\n"
            "**Email Structure & Rules:**\n"
            "1.  **Subject Line:** Create a short, intriguing, all-lower-case subject line. It should be relevant to the hook.\n"
            "2.  **Opening:** Start with 'Hi [Prospect First Name],'. Immediately use the provided personalized hook to capture their attention.\n"
            "3.  **The Bridge (1-2 sentences):** Briefly and clearly connect the hook to the value proposition. Introduce 'Hyperion Agency' as a firm that builds autonomous AI agents to solve complex business problems, similar to the research agent that found this information.\n"
            "4.  **Call to Action (CTA - 1 sentence):** End with a single, low-friction, open-ended question. Do NOT ask for a meeting. Examples: 'Curious to see how it works?' or 'Mind if I share a brief on how we built the agent that found this?'\n"
            "5.  **Closing:** Use 'Best,' followed by 'Aaryan'.\n\n"
            "**Input Data:**\n"
            f"- Prospect First Name: {prospect_name}\n"
            f"- Prospect Title: {prospect_title}\n"
            f"- Company Name: {company_name}\n"
            f"- Personalized Hook: {hook}\n\n"
            "**Output Format:**\n"
        ...
        print(f" - An error occurred during email generaion: {e}")
        return None